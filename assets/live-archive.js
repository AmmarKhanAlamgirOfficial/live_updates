document.addEventListener('DOMContentLoaded', () => {
    ...
    // 4. Load More Pagination Logic
    const loadMoreBtn = document.getElementById('load-more-btn');
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            // Find all currently hidden posts
            const hiddenPosts = document.querySelectorAll('.live-post.hidden-post');
            const postsToReveal = 100; // Amount of posts to load per click
            ...
            if (document.querySelectorAll('.live-post.hidden-post').length === 0) {
